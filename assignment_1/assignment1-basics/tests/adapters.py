...
def run_multihead_self_attention(
    ...
    if in_features.dim() == 2:
        ...
        # 传递 user expecting (x, wq, wk, wv, wo)
        # q_proj_weight is (d_k, d_model), matches what user code implies for x @ wq.T to yield (..., d_k)
        result = mha(in_features, q_proj_weight, k_proj_weight, v_proj_weight, o_proj_weight)
        return result.squeeze(0)
    else:
        return mha(in_features, q_proj_weight, k_proj_weight, v_proj_weight, o_proj_weight)
...
def run_multihead_self_attention_with_rope(
    ...
    Returns:
        ...
        implementation with the given QKV projection weights and input features.
    """
    from cs336_basics.model import MultiHeadAttentionWithRoPE
    
    mha_rope = MultiHeadAttentionWithRoPE(d_model, num_heads, theta, max_seq_len)
    ...
    if in_features.dim() == 2:
        in_features = in_features.unsqueeze(0)
        result = mha_rope(in_features, token_positions)
        return result.squeeze(0)
    else:
...
def run_transformer_block(
    ...
    block = TransformerBlock(
        ...
        attn_o_proj_weight=attn_o_proj_weight,
...
def run_transformer_lm(
    ...
    """
    from cs336_basics.model import TransformerLanguageModel
